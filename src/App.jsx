...
function loadTopics() {
  try { const s = localStorage.getItem("lds-topics"); if (s) return JSON.parse(s); } catch(e) {}
  return DEFAULT_TOPICS;
}
function saveTopics(t) { try { localStorage.setItem("lds-topics", JSON.stringify(t)); } catch(e) {} }

function loadIdeas() {
  try {
    const s = localStorage.getItem("lds-journal-ideas");
    if (!s) return [];
    return JSON.parse(s).map(idea => {
      if (!idea.notes) return { ...idea, notes: [{ id: idea.id+"_0", text: idea.text||"", date: idea.date }] };
      return idea;
    });
  } catch(e) { return []; }
}
function saveIdeas(i) { try { localStorage.setItem("lds-journal-ideas", JSON.stringify(i)); } catch(e) {} }
...
{
  "scriptures": [{ "reference": "string", "text": "under 20 words", "connection": "under 15 words" }],
  "talks": [{ "title": "string", "speaker": "string", "year": "string", "connection": "under 15 words" }],
  "stories": [{ "title": "string", "content": "2 sentences max", "source": "string" }],
  "philosophy": [{ "quote": "under 25 words", "author": "string", "connection": "under 15 words" }]
}

Include exactly: 3 scriptures from across all standard works, 3 General Conference talks (real, with correct speakers), 2 stories or illustrations, 2 philosophy quotes from Christian intellectuals such as C.S. Lewis, G.K. Chesterton, Dietrich Bonhoeffer, Tim Keller, Dallas Willard, N.T. Wright, or similar thinkers whose ideas complement LDS doctrine. Be brief. Valid JSON only.`;

const BUILD_PROMPT = `You are an LDS talk builder. You receive a speaker's full chain of voice notes on a single gospel subtopic — treat them as a connected web of evolving thought and synthesize everything into ONE unified, coherent talk. Do not list notes as separate points.
  ...
  "sections": [
    {
      ...
      "heading": "string",
      "bullets": ["3-6 word scannable phrase"],
      "fullText": "string — natural spoken paragraphs, warm and personal, as long as needed to fully develop the point"
    }
  ]
}

Section counts by time:
- 5 min: opening, 1 point, 1 scripture, testimony (4 sections)
- 10 min: opening, 2 points, 2 scriptures, 1 story, testimony (6 sections)
- 15 min: opening, 3 points, 3 scriptures, 2 stories, 1 conference talk, testimony (8 sections)
- 20 min: opening, 3 points, 4 scriptures, 2 stories, 2 conference talks, testimony (10 sections)
...
async function fetchEnrichment(notes, topicLabel, subtopic) {
  ...
  return callClaude({
    model: "claude-sonnet-4-6", max_tokens: 2000, system: ENRICH_PROMPT,
    messages: [{ role:"user", content:`Primary topic: ${topicLabel}\nSubtopic: ${subtopic}\n\n${combined}` }]
  });
}

async function buildTalk(idea, minutes, topicLabel, selectedResources) {
  const notes = idea.notes.map((n,i)=>`Note ${i+1}: "${n.text}"`).join("\n");
  let resourceStr = "";
  if (selectedResources && (selectedResources.scriptures?.length || selectedResources.talks?.length || selectedResources.stories?.length || selectedResources.philosophy?.length)) {
    resourceStr = `\n\nSelected resources to weave in naturally:\n`;
    if (selectedResources.scriptures?.length) resourceStr += `Scriptures: ${JSON.stringify(selectedResources.scriptures)}\n`;
    if (selectedResources.talks?.length) resourceStr += `Conference Talks: ${JSON.stringify(selectedResources.talks)}\n`;
    if (selectedResources.stories?.length) resourceStr += `Stories: ${JSON.stringify(selectedResources.stories)}\n`;
    if (selectedResources.philosophy?.length) resourceStr += `Philosophy: ${JSON.stringify(selectedResources.philosophy)}\n`;
  }
  return callClaude({
    model: "claude-sonnet-4-6", max_tokens: 6000, system: BUILD_PROMPT,
    messages: [{ role:"user", content:`Primary topic: ${topicLabel}\nSubtopic: ${idea.subtopic||"General"}\nTarget time: ${minutes} minutes\n\nFull idea note chain (synthesize into one unified talk):\n${notes}${resourceStr}` }]
  });
}
...
function EnrichmentView({ data, selectable=false, selected=null, onToggle=null }) {
  const sections = [
    { key:"scriptures", icon:"📖", label:"Scriptures", renderItem:(s,i)=>(
      <div key={i} style={{backgroundColor:C.softGold,borderRadius:12,padding:"12px 14px",marginBottom:8,borderLeft:`3px solid ${C.gold}`,display:"flex",gap:10,alignItems:"flex-start"}}>
        {selectable&&<input type="checkbox" checked={selected?.scriptures?.includes(i)||false} onChange={()=>onToggle("scriptures",i)} style={{marginTop:3,flexShrink:0,width:16,height:16,cursor:"pointer"}} />}
        <div style={{flex:1}}>
    ...
    { key:"talks", icon:"🎙️", label:"Conference Talks", renderItem:(t,i)=>(
      <div key={i} style={{backgroundColor:C.white,borderRadius:12,padding:"12px 14px",marginBottom:8,border:`1px solid ${C.border}`,display:"flex",gap:10,alignItems:"flex-start"}}>
        {selectable&&<input type="checkbox" checked={selected?.talks?.includes(i)||false} onChange={()=>onToggle("talks",i)} style={{marginTop:3,flexShrink:0,width:16,height:16,cursor:"pointer"}} />}
        <div style={{flex:1}}>
    ...
    { key:"stories", icon:"📚", label:"Stories & Examples", renderItem:(s,i)=>(
      <div key={i} style={{backgroundColor:C.white,borderRadius:12,padding:"12px 14px",marginBottom:8,border:`1px solid ${C.border}`,display:"flex",gap:10,alignItems:"flex-start"}}>
        {selectable&&<input type="checkbox" checked={selected?.stories?.includes(i)||false} onChange={()=>onToggle("stories",i)} style={{marginTop:3,flexShrink:0,width:16,height:16,cursor:"pointer"}} />}
        <div style={{flex:1}}>
    ...
    { key:"philosophy", icon:"💭", label:"Christian & Philosophical Insights", renderItem:(p,i)=>(
      <div key={i} style={{backgroundColor:C.softGold,borderRadius:12,padding:"12px 14px",marginBottom:8,borderLeft:`3px solid ${C.border}`,display:"flex",gap:10,alignItems:"flex-start"}}>
        {selectable&&<input type="checkbox" checked={selected?.philosophy?.includes(i)||false} onChange={()=>onToggle("philosophy",i)} style={{marginTop:3,flexShrink:0,width:16,height:16,cursor:"pointer"}} />}
        <div style={{flex:1}}>
      ...
      <div style={{backgroundColor:C.navy,padding:"14px 20px",flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          <div style={{display:"flex",backgroundColor:"rgba(255,255,255,0.1)",borderRadius:20,padding:3}}>
            {["bullets","full"].map(v=>(
              <button key={v} onClick={()=>setView(v)} style={{padding:"6px 14px",borderRadius:17,border:"none",cursor:"pointer",backgroundColor:view===v?C.gold:"transparent",color:view===v?C.navy:C.gold,fontSize:12,fontWeight:700,transition:"all 0.2s",WebkitTapHighlightColor:"transparent"}}>
                {v==="bullets"?"Bullets":"Full Text"}
              </button>
            ))}
          </div>
        </div>
          ...
          return (
            <div key={i} style={{marginBottom:24}}>
              ...
              {view==="bullets"?(
                ...
                <p style={{color:C.navy,fontSize:17,lineHeight:1.8,fontFamily:section.type==="scripture"?"Georgia, serif":"inherit",fontStyle:section.type==="scripture"?"italic":"normal",margin:0,backgroundColor:section.type==="scripture"?C.softGold:"transparent",borderLeft:section.type==="scripture"?`3px solid ${C.gold}`:"none",borderRadius:section.type==="scripture"?"0 8px 8px 0":0,padding:section.type==="scripture"?"10px 12px":0}}>
                  {section.fullText}
                </p>
              )}
            </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"20px 20px 40px"}}>
        <div style={{backgroundColor:C.white,borderRadius:16,padding:16,marginBottom:20,boxShadow:"0 2px 8px rgba(27,42,74,0.07)"}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:8}}>
            <input value={newLabel} onChange={e=>setNewLabel(e.target.value)} placeholder="Topic name..." style={{flex:1,padding:"10px 12px",borderRadius:10,border:`1.5px solid ${C.border}`,fontSize:14,color:C.navy,outline:"none"}} />
            <input type="color" value={newColor} onChange={e=>setNewColor(e.target.value)} style={{width:44,height:44,borderRadius:10,border:`1.5px solid ${C.border}`,cursor:"pointer",padding:2}} />
          </div>
          <button onClick={addTopic} style={{width:"100%",padding:"10px",borderRadius:10,border:"none",backgroundColor:C.navy,color:C.gold,fontSize:13,fontWeight:700,cursor:"pointer"}}>+ Add Topic</button>
        </div>
            ...
            <div key={topic.id} style={{backgroundColor:C.white,borderRadius:14,padding:"12px 14px",boxShadow:"0 1px 6px rgba(27,42,74,0.06)",borderLeft:`4px solid ${topic.color}`}}>
              {editingId===topic.id?(
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <input value={topic.label} onChange={e=>updateLabel(topic.id,e.target.value)} style={{flex:1,padding:"8px 10px",borderRadius:8,border:`1.5px solid ${C.border}`,fontSize:13,color:C.navy,outline:"none"}} />
                  <input type="color" value={topic.color} onChange={e=>updateColor(topic.id,e.target.value)} style={{width:36,height:36,borderRadius:8,border:`1.5px solid ${C.border}`,cursor:"pointer",padding:2}} />
...
function initSelected(enrichment) {
  if (!enrichment) return { scriptures:[], talks:[], stories:[], philosophy:[] };
  return {
    scriptures: enrichment.scriptures?.map((_,i)=>i)||[],
    talks:      enrichment.talks?.map((_,i)=>i)||[],
    stories:    enrichment.stories?.map((_,i)=>i)||[],
    philosophy: enrichment.philosophy?.map((_,i)=>i)||[],
  };
}
...
function BuildMode({ ideas, topics }) {
  ...
  const selectIdea = (idea) => {
    ...
    setResourceSelected(initSelected(idea.enrichment));
  };

  const toggleResource = (type, idx) => {
    setResourceSelected(prev => {
      const arr = prev[type].includes(idx) ? prev[type].filter(i=>i!==idx) : [...prev[type],idx];
      return { ...prev, [type]: arr };
    });
  };

  const getSelectedResources = () => {
    if (!selectedIdea?.enrichment || !resourceSelected) return null;
    const e = selectedIdea.enrichment;
    return {
      scriptures: resourceSelected.scriptures.map(i=>e.scriptures[i]).filter(Boolean),
      talks:      resourceSelected.talks.map(i=>e.talks[i]).filter(Boolean),
      stories:    resourceSelected.stories.map(i=>e.stories[i]).filter(Boolean),
      philosophy: resourceSelected.philosophy.map(i=>e.philosophy[i]).filter(Boolean),
    };
  };
  ...
  const handleBuild = async () => {
    ...
    try {
      ...
      const result = await buildTalk(selectedIdea, minutes, topic?.label||selectedIdea.topicId, selected);
      setTalk(result);
    } catch(e) {
  ...
  if (talk) return <TalkView talk={talk} onClose={()=>{setTalk(null);setSelectedIdea(null);setResourceSelected(null);}} />;

  const filtered = filterTopic ? ideas.filter(i=>i.topicId===filterTopic) : ideas;
  const topicsWithIdeas = topics.filter(t=>ideas.some(i=>i.topicId===t.id));
  ...
  return (
    <div style={{padding:"20px 20px 60px"}}>
      ...
      {!selectedIdea ? (
        <div>
          <div style={{fontSize:11,fontWeight:700,color:C.navy,opacity:0.5,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:14}}>Choose an idea</div>
          {ideas.length===0 ? (
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {filtered.map(idea=>{
                ...
                return (
                  <button key={idea.id} onClick={()=>selectIdea(idea)} style={{textAlign:"left",backgroundColor:C.white,borderRadius:16,padding:"14px 16px",border:`2px solid ${C.border}`,cursor:"pointer",WebkitTapHighlightColor:"transparent",boxShadow:"0 2px 8px rgba(27,42,74,0.06)",borderLeft:`5px solid ${topic?.color||C.gold}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                      <div>
                        ...
                        {idea.subtopic&&<span style={{fontSize:12,fontWeight:700,color:C.navy,marginLeft:6}}>· {idea.subtopic}</span>}
                      </div>
                      <span style={{fontSize:10,color:C.navy,opacity:0.35}}>{idea.notes?.length} note{idea.notes?.length!==1?"s":""}{idea.enrichment?" · ✦":""}</span>
                    </div>
                    <p style={{color:C.navy,fontSize:14,lineHeight:1.55,fontFamily:"Georgia, serif",margin:0}}>
        ...
        <div>
          <button onClick={()=>{setSelectedIdea(null);setResourceSelected(null);}} style={{background:"none",border:"none",color:C.navy,opacity:0.5,fontSize:13,fontWeight:600,cursor:"pointer",padding:0,marginBottom:16,WebkitTapHighlightColor:"transparent"}}>← Back to ideas</button>

          {/* Selected idea summary */}
          {(()=>{
            const topic = topics.find(t=>t.id===selectedIdea.topicId);
            return (
              <div style={{backgroundColor:C.white,borderRadius:16,padding:"14px 16px",marginBottom:20,borderLeft:`5px solid ${topic?.color||C.gold}`,boxShadow:"0 2px 8px rgba(27,42,74,0.06)"}}>
                <div style={{fontSize:10,fontWeight:700,color:topic?.color||C.gold,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>{topic?.label}</div>
                {selectedIdea.subtopic&&<div style={{fontSize:15,fontWeight:700,color:C.navy,fontFamily:"Georgia, serif",marginBottom:4}}>{selectedIdea.subtopic}</div>}
                <div style={{fontSize:12,color:C.navy,opacity:0.5}}>{selectedIdea.notes?.length} voice note{selectedIdea.notes?.length!==1?"s":""}</div>
              </div>
            );
          })()}

          {/* Resource selection */}
          {selectedIdea.enrichment && resourceSelected ? (
            <div style={{marginBottom:20}}>
              <div style={{fontSize:11,fontWeight:700,color:C.navy,opacity:0.5,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:4}}>Select resources to include</div>
              <div style={{fontSize:12,color:C.navy,opacity:0.4,marginBottom:12}}>Check the ones you want woven into the talk</div>
              <EnrichmentView data={selectedIdea.enrichment} selectable={true} selected={resourceSelected} onToggle={toggleResource} />
            </div>
          ) : !selectedIdea.enrichment ? (
            <div style={{backgroundColor:C.softGold,borderRadius:12,padding:"12px 14px",marginBottom:20,borderLeft:`3px solid ${C.gold}`}}>
              <p style={{color:C.navy,fontSize:13,margin:0,lineHeight:1.5}}>💡 Tip: Go to the Ideas tab and tap <strong>✦ Enrich</strong> on this idea first to add scriptures, conference talks, and stories you can select for the talk.</p>
            </div>
          ) : null}

          {/* Time selector */}
          <div style={{fontSize:11,fontWeight:700,color:C.navy,opacity:0.5,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:14}}>How long is your talk?</div>
          <div style={{display:"flex",gap:10,marginBottom:8}}>
            {[5,10,15,20].map(m=>(
              <button key={m} onClick={()=>setMinutes(m)} style={{flex:1,padding:"14px 0",borderRadius:14,backgroundColor:minutes===m?C.navy:C.white,color:minutes===m?C.gold:C.navy,fontSize:15,fontWeight:700,border:`2px solid ${minutes===m?C.navy:C.border}`,cursor:"pointer",WebkitTapHighlightColor:"transparent",transition:"all 0.15s"}}>
                {m}<span style={{fontSize:11}}>m</span>
              </button>
            ))}
          </div>
          <p style={{color:C.navy,opacity:0.4,fontSize:12,marginBottom:20,textAlign:"center"}}>
            {minutes===5?"1 point · 1 scripture · testimony":minutes===10?"2 points · 2 scriptures · 1 story · testimony":minutes===15?"3 points · 3 scriptures · 2 stories · 1 talk · testimony":"3 points · 4 scriptures · 2 stories · 2 talks · testimony"}
          </p>
          {error&&<p style={{color:"#DC2626",fontSize:13,textAlign:"center",marginBottom:14}}>{error}</p>}
          <button onClick={handleBuild} disabled={building} style={{width:"100%",padding:18,borderRadius:16,border:"none",backgroundColor:building?C.border:C.gold,color:building?"#999":C.navy,fontSize:17,fontWeight:700,cursor:building?"not-allowed":"pointer",fontFamily:"Georgia, serif",WebkitTapHighlightColor:"transparent",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
...
export default function App() {
  ...
  const saveNewIdea = () => {
    ...
    const idea = { id:Date.now(), topicId:selectedTopicId, subtopic:subtopic.trim(), date:now, notes:[{id:Date.now()+"_0",text,date:now}], enrichment:null };
  ...
  return (
    <div style={{minHeight:"100vh",backgroundColor:C.parchment,fontFamily:"system-ui, -apple-system, sans-serif",maxWidth:480,margin:"0 auto"}}>
      {showTopicsManager&&<TopicsManager topics={topics} onSave={updateTopics} onClose={()=>setShowTopicsManager(false)} />}
      ...
      <div style={{backgroundColor:C.navy,padding:"16px 20px 0",position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <button onClick={()=>setShowTopicsManager(true)} style={{background:"none",border:`1px solid rgba(201,168,76,0.4)`,borderRadius:12,color:C.gold,fontSize:11,fontWeight:700,padding:"5px 10px",cursor:"pointer",WebkitTapHighlightColor:"transparent"}}>Topics</button>
        </div>
        <div style={{display:"flex",borderBottom:`2px solid rgba(255,255,255,0.08)`}}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setMode(t.id)} style={{flex:1,padding:"10px 4px",border:"none",cursor:"pointer",backgroundColor:"transparent",color:mode===t.id?C.gold:"rgba(255,255,255,0.45)",fontSize:13,fontWeight:700,borderBottom:`2px solid ${mode===t.id?C.gold:"transparent"}`,marginBottom:-2,transition:"all 0.2s",WebkitTapHighlightColor:"transparent"}}>{t.label}</button>
          ))}
        </div>
      </div>
          ...
          {showSave&&capture.transcript.trim()&&(
            <div style={{width:"100%"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:20}}>
                {topics.map(t=>(
                  <button key={t.id} onClick={()=>setSelectedTopicId(t.id)} style={{padding:"9px 14px",borderRadius:20,border:`2px solid ${selectedTopicId===t.id?t.color:C.border}`,backgroundColor:selectedTopicId===t.id?t.color:"transparent",color:selectedTopicId===t.id?C.white:C.navy,fontSize:12,fontWeight:600,cursor:"pointer",transition:"all 0.15s",WebkitTapHighlightColor:"transparent"}}>{t.label}</button>
                ))}
              </div>
              ...
              <input value={subtopic} onChange={e=>setSubtopic(e.target.value)} placeholder="e.g. Bread of Life, Faith in Dark Times..." style={{width:"100%",padding:"12px 14px",borderRadius:12,border:`1.5px solid ${C.border}`,fontSize:14,color:C.navy,marginBottom:20,boxSizing:"border-box",outline:"none",backgroundColor:C.white}} />
              <div style={{display:"flex",gap:12}}>
